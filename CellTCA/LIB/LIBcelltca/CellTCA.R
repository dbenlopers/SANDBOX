...
#-------------------------------------------------------------------------------#
# CellTCA::getPlateNormGeneBasedData function
#-------------------------------------------------------------------------------#
setGeneric("getPlateNormGeneBasedData",
           function(object) {
             standardGeneric("getPlateNormGeneBasedData")
           }
)

setMethod("getPlateNormGeneBasedData",
          signature = signature("CellTCA"),
          definition = function(object) {
...
#-------------------------------------------------------------------------------#
setGeneric("getWellNormGeneBasedData",
           function(object, ...) {
             standardGeneric("getWellNormGeneBasedData")
           }
)

setMethod("getWellNormGeneBasedData",
          signature = signature("CellTCA"),
          definition = function(object, geneSVMLabels) {
...
#-------------------------------------------------------------------------------#
# CellTCA::getWellNormGeneBasedDataNew function
#-------------------------------------------------------------------------------#
setGeneric("getSimpleWellNormGeneBasedDataNew",
           function(object) {
             standardGeneric("getSimpleWellNormGeneBasedDataNew")
           }
)

setMethod("getSimpleWellNormGeneBasedDataNew",
          signature = signature("CellTCA"),
          definition = function(object) {
            # No intra plate normalization available
            if (getNumberOfReplicates(object) == 1)
            ...
            for (k in 1:length(cols)) {
              ...
              for (i in 1:length(geneNames)) {   
                ...
                if (is.matrix(dataForNorm)) {
                  ...
                  if (max(naCount) > 0.75*nrow(dataForNorm))
                    normalizedData <- dataForNorm
                  else
                    normalizedData <- normalizeBetweenArrays(dataForNorm, method="quantile")
                  #return(dataForNorm)
...
#-------------------------------------------------------------------------------#
# CellTCA::getSimpleWellPCANormGeneBasedData function
#-------------------------------------------------------------------------------#
setGeneric("getSimpleWellPCANormGeneBasedData",
           function(object, cols) {
             standardGeneric("getSimpleWellPCANormGeneBasedData")
           }
)

setMethod("getSimpleWellPCANormGeneBasedData",
          signature = signature("CellTCA"),
          definition = function(object, cols) {
            # No intra plate normalization available
            if (getNumberOfReplicates(object) == 1)
              return(getGeneBasedData(object, 1))
            ...
            dataToSummarize <- as.data.frame(apply(dataToSummarize, 2, function(x) (x-mean(x, na.rm=T))/sd(x, na.rm=T)))
            # Collect gene names for entries in the global data frame
            dataToSummarize$GeneName <- rep(geneNames, geneCnt)
            dataToSummarize <- new("TCAData", data=dataToSummarize)
            return(getGeneBasedData(dataToSummarize))
...
# 
# #-------------------------------------------------------------------------------#
# # CellTCA::getWellNormGeneBasedDataOld function
# #-------------------------------------------------------------------------------#
# setGeneric("getWellNormGeneBasedDataOld",
#            function(object, ...) {
#              standardGeneric("getWellNormGeneBasedDataOld")
#            }
# )
# 
# setMethod("getWellNormGeneBasedDataOld",
#           signature = signature("CellTCA"),
#           definition = function(object, geneSVMLabels) {
#             # No intra plate normalization available
#             if (getNumberOfReplicates(object) == 1)
...
#                   
#                   if (valid & length(dataTmp[, cols[k]]) & length(which(!is.na(dataTmp[, cols[k]])))) {
#                     # Making sure that column is not an na column
#                     if (delCol != 2) {
#                       dataForNorm[1:nrow(dataTmp), j-delCol] <- as.numeric(dataTmp[1:nrow(dataTmp), cols[k]])
#                     }
#                     else {
#                       dataForNorm[1:nrow(dataTmp)] <- as.numeric(dataTmp[1:nrow(dataTmp), cols[k]])
#                     }
#                   }
#                   else {
...
# #-------------------------------------------------------------------------------#
# # CellTCA::getSimpleWellNormGeneBasedData function
# #-------------------------------------------------------------------------------#
# setGeneric("getSimpleWellNormGeneBasedData",
#            function(object) {
#              standardGeneric("getSimpleWellNormGeneBasedData")
#            }
# )
# 
# setMethod("getSimpleWellNormGeneBasedData",
#           signature = signature("CellTCA"),
#           definition = function(object) {
#             # No intra plate normalization available
#             if (getNumberOfReplicates(object) == 1)
...
# )
# 
# getSimpleWellNormGeneBasedData_c <- compiler::cmpfun(getSimpleWellNormGeneBasedData)
...
#-------------------------------------------------------------------------------#
setGeneric("getMeanAndSdCellCounts",
           function(object) {
             standardGeneric("getMeanAndSdCellCounts")
           }
)


setMethod("getMeanAndSdCellCounts",
          signature = signature("CellTCA"),
          definition = function(object) {
            geneName <- c()
            cellCount <- c()
...
setMethod("plotSingleGeneDataDistribution",
          ...
          definition = function(object, geneName, col, cutoff) {
            ...
            for (i in 1:getNumberOfReplicates(object)) { 
              ...
              plotDensity(cellMatrix, na.rm=T, lty=1, lwd=1, xlim=c(0, cutoff))
            }
          }
)
...
setMethod("plotCellNumberDistribution",
          ...
          definition = function(object, col, cutoff) {
            ...
            for (i in 1:getNumberOfReplicates(object)) { 
              ...
              plot(density(cellData, na.rm=T), lty=1, lwd=1, xlim=c(0, cutoff))
            }
          }
)
...
#-------------------------------------------------------------------------------#
setGeneric("validateTCAGeneData",
           function(object, svmModel, ...) {
             standardGeneric("validateTCAGeneData")
           }
)

setMethod("validateTCAGeneData",
          signature = signature("CellTCA"),
...
#-------------------------------------------------------------------------------#
# CellTCA::plotTCADensities function
#-------------------------------------------------------------------------------#
setGeneric("plotTCADensities",
           function(object, ...) {
             standardGeneric("plotTCADensities")
           }
)


setMethod("plotTCADensities",
          signature = signature("CellTCA"),
          definition = function(object, gData, gSData, geneRepIds, geneNum, colNum, colname, fname, wd, ht) {
            geneNames <- sort(getUniqueGeneNames(object))
            dat1 <- NULL
            dat2 <- NULL
            dat3 <- NULL
            #print(geneNames[geneNum])
            for (j in 1:getNumberOfReplicates(object)) {
              dataTmp <- as.numeric(getGeneNameTCAData(object@replicates[[j]], geneNames[geneNum], colname))
              if (length(dataTmp)) {
                if (j == 1) {
                  dat1 <- dataTmp
                  qdat <- quantile(dat1, probs=c(0.05, 0.95), na.rm=T)
                  dat1 <- dat1[which((dat1>=qdat[1])&(dat1<=qdat[2]))]
                }
                else if (j == 2) {
                  dat2 <- dataTmp
                  qdat <- quantile(dat2, probs=c(0.05, 0.95), na.rm=T)
                  dat2 <- dat2[which((dat2>=qdat[1])&(dat2<=qdat[2]))]
                }
                else if (j == 3) {
                  dat3 <- dataTmp
                  qdat <- quantile(dat3, probs=c(0.05, 0.95), na.rm=T)
                  dat3 <- dat3[which((dat3>=qdat[1])&(dat3<=qdat[2]))]
                }
                else
                  stop("validateGeneData: too many replicates")
            ...
            if (is.null(dat1))
              print("TCAData not found for plate 1")
            if (is.null(dat2))
              print("TCAData not found for plate 2")
            if (is.null(dat3))
              print("TCAData not found for plate 3")
            
            gD1 <- getContent(gData)
            gD2 <- getContent(gSData)
            
            geneId1 <- which(names(gD1) == geneNames[geneNum])
            if (length(geneId1) == 0)
              stop(paste("plotTCADensities: geneName ", geneNames[geneNum], " not found in gD1", sep=""))
            geneId2 <- which(names(gD2) == geneNames[geneNum])
            if (length(geneId2) == 0)
              stop(paste("plotTCADensities: geneName ", geneNames[geneNum], " not found in gD2", sep=""))
            gData1 <- gD1[[geneId1]][, colNum]
            qData <- quantile(gData1, probs=c(0.05, 0.95), na.rm=T)
            gData1 <- gData1[which((gData1 >= qData[1]) & (gData1 <= qData[2]))]
            gData2 <- gD2[[geneId2]][, colNum]
            qData <- quantile(gData2, probs=c(0.05, 0.95), na.rm=T)
            gData2 <- gData2[which((gData2 >= qData[1]) & (gData2 <= qData[2]))]
            maxL <- max(length(dat1), length(dat2), length(dat3), length(gData1), length(gData2))
            mat <- matrix(, maxL, 5)
            excludeIdx <- c()
            
            if (is.null(dat1))
              excludeIdx <- c(excludeIdx, 1)
            else 
              mat[1:length(dat1), 1] <- dat1
            if (is.null(dat2))
              excludeIdx <- c(excludeIdx, 2)
            else
              mat[1:length(dat2), 2] <- dat2
            if (is.null(dat3))
              excludeIdx <- c(excludeIdx, 3)
            else
              mat[1:length(dat3), 3] <- dat3
            mat[1:length(gData2), 5] <- gData2
            if (sum(geneRepIds) == 0)
              excludeIdx <- c(excludeIdx, 4)
            else
              mat[1:length(gData1), 4] <- gData1
            
            if (length(excludeIdx)) 
              mat <- mat[, -excludeIdx]
            dim(mat) <- c(maxL, 5-length(excludeIdx))
            cols <- c("black", "green", "brown", "blue", "red")
            if (length(excludeIdx))
              cols <- cols[-excludeIdx]
            #pdf(fname, width=wd, height=ht)
            plotDensity(mat, ylab = "density", xlab=geneNames[geneNum], type="l", lty="solid",col=cols, na.rm = TRUE)
            #dev.off()
          }
)
