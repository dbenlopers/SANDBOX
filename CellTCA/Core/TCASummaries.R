...
setMethod("plotSummary",
          signature = signature("TCASummaries"), 
          function (object, columnId, columnName, sampName, threshold) {
            ...
            abline(h=threshold,lty=4,lwd=1, col="purple3")
            text(max(MidPts)*1.005,threshold,"threshold used",col="purple3",adj=1,pos=3,cex=0.75)
...
setMethod("plotPlainSummary",
          signature = signature("TCASummaries"), 
          function (object, columnId, columnName) {
            par(mar = c(6.5,3.9,3,0) +0)
...
setMethod("plotCV",
          signature = signature("TCASummaries"),
          definition = function(object, plateSetup, sampName, column, colName) {
            ...
            plotPlate(as.vector(t(plate)), ncol=ncol(plate), nrow=nrow(plate),
                      cex.char=0.8, na.action="xout", main=tit, 	
...
setMethod("barPlots",
          signature = signature("TCASummaries"),
          definition = function(object, gbobject, columnId, columnName, sampName, threshold, fname, wd, ht, cnt, pSamp) {
            ...
            plotPercentData(gbobject, object, threshold, columnId, columnName, cnt, pSamp)
            dev.off()
          }
)
...
setMethod("cvPlot",
          signature = signature("TCASummaries"),
          definition = function(object, plateSetup, sampName, column, colName, fname, wd, ht) {
            pdf(fname, width=wd, height=ht)
            plotCV(object, plateSetup, sampName, column, colName)
            dev.off()
          }
)
...
setMethod("spatialSummaryPlot",
          ...
          definition = function(object, plateSetup, sampName, column, colName, fname, wd, ht) {
            pdf(fname, width=wd, height=ht)
            plotSpatialSummary(object, plateSetup, sampName, column, colName)
            dev.off()
          }
)  
...
setMethod("plotSpatialSummary",
          signature = signature("TCASummaries"), 
          function (object, plateSetup, sampName, columnId, columnName) {
            tit <- paste(c("Summary values distribution per Well for ", columnName), collapse="")
            plateS <- getContent(plateSetup)
            plate <- matrix(, nrow(plateS), ncol(plateS))
            averages <- getMedianSummary(object, columnId)
...
setMethod("plotToxIndex",
          signature = signature("TCASummaries"), 
          function (object, values, column, fname, wd, ht, count, ...) {
            pdf(fname, width=wd, height=ht)
...
setMethod("plotRelativeRiskSpatialSummaryHelper",
          ...
          function (object, gobject, plateSetup, sampName, columnId, columnName, threshold, addtitle=1) {
            ...
            percent_data <- log(percent_data) - log(percent_data[sampName])
            ...
            if (length(sname) > 2) 
              sname <- "CT"
            labels[which(as.vector(t(plateS))==sampName)] <- sname
            plotPlate(as.vector(t(plate)), ncol=ncol(plate), nrow=nrow(plate), col=brewer.pal(11, "Spectral"), main=tit, na.action="xout");
          }
)  
...
setMethod("plotProbRelativeRiskSpatialSummaryHelper",
          ...
          function (object, gobject, plateSetup, sampName, columnId, columnName, threshold, addtitle=1) {
            ...
            percent_data <- log(mean_data) - log(mean_data[sampName])  	
            #percent_data <- percent_data - min(percent_data)
            #print(percent_data)
            plate[,] <- percent_data[match(plateS, names(percent_data))]
            labels <- rep(NA_character_, length(plateS))
