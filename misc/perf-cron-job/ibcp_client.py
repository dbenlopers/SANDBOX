...
LOGGER = logging.getLogger('ibcp.datalake.client')
...
class GIBSDataRetriever():
    ...
    def __init__(self, ws_root_url, db_user, db_password, allow_purge=False):
    ...
    def iterate(self, method, action, entity, params={}):
        LOGGER.info('Querying %s/%s' % (action, entity))
    ...
    def import_results(self, last_pull_date, period=900):
        start = (last_pull_date if isinstance(last_pull_date, datetime) else
                 parse_date(last_pull_date))
        ...
        return self.iterate(
            'get', 'find', Entities.Results.value, params)
